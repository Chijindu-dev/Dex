// scripts/deployTempo.cjs - Deployment for Tempo testnet
const hre = require("hardhat");
const ethers = require("ethers");

async function main() {
  console.log("🚀 Deploying DEX to Tempo Testnet (Moderato)...\n");

  // Get signers using ethers provider
  const provider = new ethers.JsonRpcProvider(process.env.TEMPO_RPC_URL || "https://rpc.moderato.tempo.xyz");
  ...
  if (balance === BigInt(0)) {
    console.error("❌ Error: Account has no ETH on Tempo testnet.");
    console.log("Wallet address:", wallet.address);
    console.log("Please get testnet ETH from Tempo testnet faucet");
    process.exit(1);
  }

  // Step 1: Deploy DexFactory
  ...
  console.log("\n" + "=" .repeat(50));
  console.log("STEP 3: Deploying Test Tokens");
  console.log("=" .repeat(50));
  ...
  console.log("\n🌐 Tempo Testnet Explorer:");
  console.log("   https://explore.tempo.xyz");
}
